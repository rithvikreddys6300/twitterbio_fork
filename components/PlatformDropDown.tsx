...
export default function PlatformDropDown({ platform, setPlatform }: PlatformDropDownProps) {
  ...
  return (
    <Menu as="div" className="relative block text-left w-full">
      ...
      >
        <Menu.Items
          className="absolute left-0 z-10 mt-2 w-full origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none max-h-60 overflow-auto"
          key={platform}
        >
          <div className="">
            {platforms.map((platformItem) => (
              <Menu.Item key={platformItem}>
                {({ active }) => (
                  ...
                  >
                    <span>{PLATFORM_CONFIGS[platformItem].displayName}</span>
